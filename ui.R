...
tag_twitter <- function(){
  t <- tags$a(href=url1, "Tweet", class="twitter-share-button")
...
div_instructions <- function(){
  dv <- div(
    tags$ol(
      ...
      tags$li("Algunos congresistas tienen twitter. Para ellos te preparamos un link, cópialo y pégalo en tu browser ;)")
    ))
  return(dv)
}
...
div_explicacion <- function(){
  dv <- div(h5("El lunes 09 el Congreso votará la vacancia del presidente Martín Vizcarra por segunda vez. 
            Creemos que si bien los hechos meritan una investigación y posible sanción estos no justifican la vacancia del presidente especialmente en plena pandemia y a poco meses de una nueva elección. 
